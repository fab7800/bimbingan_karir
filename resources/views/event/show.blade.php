<x-layouts.app>
  <section class="max-w-7xl mx-auto py-12 px-6">
    ...
    <div id="checkoutAlert" class="toast toast-top toast-center hidden z-50">
      <div class="alert alert-error">
        <span id="checkoutAlertMessage"></span>
      </div>
    </div>
            ...
            @php
              ...
              if ($imagePath) {
                if (filter_var($imagePath, FILTER_VALIDATE_URL)) {
                  ...
                  $imageUrl = asset('images/events/' . $imagePath);
                }
              } else {
                $imageUrl = 'https://img.daisyui.com/images/stock/photo-1606107557195-0e29a4b5b4aa.webp';
              }
            @endphp
            <img src="{{ $imageUrl }}" alt="{{ $event->judul }}" class="w-full h-96 object-cover" />
          </figure>
          <div class="card-body">
  ...
  <script>
    ...
    document.getElementById('confirmCheckout').addEventListener('click', async () => {
      ...
      if (items.length === 0) {
        showCheckoutAlert('Tidak ada tiket dipilih');
      ...
      try {
        ...
        console.log(err);
        showCheckoutAlert('Terjadi kesalahan saat memproses pesanan: ' + err.message);
        btn.removeAttribute('disabled');
        btn.textContent = 'Konfirmasi';
      }
    });
